...
@router.get("/applicable/{osm_type}/{osm_id}", response_model=QuestApplicableListResponse)
async def get_applicable_quests_for_poi(
    ...
    logger.info(f"Fetching OSM data for {osm_type}/{osm_id} to determine applicable quests")
    ...
    try:
        ...
        logger.info(f"OSM data fetched successfully for {osm_type}/{osm_id}, tags: {osm_tags}")
    ...
    logger.info(f"Found {len(applicable_quests)} applicable quests for {osm_type}/{osm_id}")
    ...
    logger.info(f"Returning {len(quest_responses)} applicable quests for {osm_type}/{osm_id}")
...
async def respond_to_quest(
    ...
    poi = db.query(POI).filter(
        POI.osm_type == request.poi_osm_type,
        POI.osm_id == request.poi_osm_id
    ).first()
